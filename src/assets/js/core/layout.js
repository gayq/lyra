...
let fallRuntime = null;

function resetFallPool() {
...
export function fallOnBrowserView() {
    if (!fallRuntime) return;
    stopFallLoop();
    resetFallPool();
}

export function fallOnHomeView() {
    if (!fallRuntime) return;
    stopFallLoop();
    ...
    fallRuntime.rafId = requestAnimationFrame(function fallTick(time) {
        ...
        if (document.hidden || document.body.classList.contains('browser-view')) return;
...
function spawnFallImage() {
    if (!fallRuntime) return;
    const { pool } = fallRuntime;
    let img = null;
    for (let i = 0; i < pool.length; i++) {
        if (pool[i].style.display === 'none') {
            img = pool[i];
            break;
        }
    }
