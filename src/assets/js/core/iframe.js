...
export function updateHistoryUI(activeTab, { currentUrl, canGoBack, canGoForward }) {
    ...
    if (dom.searchInputNav) {
        ...
        if (displayText === decoded && activeTab.gameDisplayByUrl && typeof activeTab.gameDisplayByUrl === 'object') {
            const normalized = normalizeGameHistoryUrl(decoded);
            const remembered = normalized ? activeTab.gameDisplayByUrl[normalized] : null;
        ...
        if (displayText === decoded && activeTab.fixedTitle) {
            ...
            if (safeTitle && safeTitle !== 'fetching data...' && safeTitle !== 'new tab') {
                const source = (localStorage.getItem('gameSource') || 'gn-math').toLowerCase();
