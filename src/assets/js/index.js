...
import { initializeIframe, updateHistoryUI, cleanupIframe, reduceIframeMemory } from './core/iframe.js';
...
const buildGameDisplayLabel = (title) => {
    ...
    const source = (localStorage.getItem('gameSource') || 'selenite').toLowerCase();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadNewTabGameData() {
        ...
        const source = (() => {
            ...
            return ['selenite', 'gn-math', 'edurocks', 'velara'].includes(stored) ? stored : 'selenite';
        })();
        let fetchPromise;
            ...
            fetchPromise = fetch(`/!!/${SOURCE_CONFIG.selenite.games}`)
                ...
                .then(data => {
                    ...
                    return gamesList
                        ...
                        const finalCover = imagePath ? `${SOURCE_CONFIG.selenite.assets}${gamePath}/${imagePath}` : null;
                        return {
                            name: game.name,
                            gameUrl: finalUrl,
                            isExternal: false,
    ...
    function addTab(url = null, title = 'new tab', isGame = false, gameIcon = null) {
    ...
    function handleNewTabAction(url, title, isGame = false, icon = null) {
        if (url) {
            const tab = addTab(url, title, isGame, icon);
            if (isGame && tab && window.WavesApp.renderTabs) {
                window.WavesApp.renderTabs();
            }
        }
        hideTabSelectionModal();
