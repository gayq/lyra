...
pub async fn route_wispnet(server: u32, packet: ConnectPacket) -> Result<ClientStream> {
    if let Some(server) = WISPNET_SERVERS.get(&server) {
        let stream = server
            ...
            .await
            .context(negative_message!("failed to connect to wispnet server"))?;
        Ok(ClientStream::Wispnet(
            Box::new(stream),
...
pub async fn handle_wispnet(stream: WispResult, id: String) -> Result<()> {
    ...
    let (mux, fut) = Box::pin(
        ClientMux::new(read, write, Some(WispV2Handshake::new(extensions)))
            .await
            .context(negative_message!("failed to create client multiplexor"))?
            .with_required_extensions(&[WispnetServerProtocolExtension::ID]),
    )
    .await
    .context(negative_message!(
        "wispnet client did not have wispnet extension"
    ))?;

    let is_private = mux
        .get_extensions()
        .find_extension::<WispnetServerProtocolExtension>()
        .context(negative_message!("failed to find wispnet extension"))?
        .1;
    ...
    match result {
        Ok(value) => debug!(
            "wispnet client id {:?} multiplexor completed {:?}{}",
            id, value, POSITIVE
        ),
        Err(error) => debug!(
            "wispnet client id {:?} multiplexor failed {:?}{}",
            id, error, NEGATIVE
        ),
    }
