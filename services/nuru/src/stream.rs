...
pub enum ClientStream {
    ...
    Udp(UdpSocket),
    #[cfg(feature = "twisp")]
    Pty(Box<(tokio::process::Child, pty_process::Pty)>),
    Wispnet(Box<MuxStream<WispStreamWrite>>, String),
...
impl ClientStream {
    pub async fn resolve(packet: ConnectPacket) -> anyhow::Result<ResolvedPacket> {
        ...
        if CONFIG.wisp.has_wispnet() && packet.host.ends_with(".wisp") {
            if let Some(wispnet_server) = packet.host.split(".wisp").next() {
                ...
                let decoded = BASE64_STANDARD
                    ...
                    .context(negative_message!("failed to decode wispnet server"))?;
                let server_id = u32::from_str(
                    &String::from_utf8(decoded)
                        .context(negative_message!("wispnet server was not a string"))?,
                )
                .context(negative_message!(
                    "failed to parse wispnet server from string"
                ))?;
        ...
        if IpAddr::from_str(&packet.host).is_ok() && !CONFIG.stream.allow_direct_ip {
            let resolved = ResolvedPacket::Blocked;
            cache.insert(cache_key.clone(), to_cached(&resolved));
            return Ok(resolved);
        }
        ...
        let resolved_iter = timeout(resolve_timeout, RESOLVER.resolve(packet.host.clone()))
            .await
            .context(negative_message!("failed to resolve hostname: timeout"))?
            .context(negative_message!("failed to resolve hostname"))?;
        let packet = resolved_iter
            ...
            .map(|addr| {
                ...
                let global = is_global(addr);
                if (global && !CONFIG.stream.allow_global)
                    || (!global && !CONFIG.stream.allow_non_global)
                {
                    return ResolvedPacket::Blocked;
                }
    ...
    pub async fn connect(packet: ConnectPacket) -> anyhow::Result<Self> {
        match packet.stream_type {
            StreamType::Tcp => {
                let ipaddr = IpAddr::from_str(&packet.host)
                    .context(negative_message!("failed to parse hostname as ip address"))?;
                let connect_timeout =
                ...
                .await
                .context(negative_message!("failed to connect to host: timeout"))?
                .with_context(|| {
                    format!(
            ...
            StreamType::Udp => {
                ...
                .await
                .context(negative_message!("failed to connect udp socket: timeout"))??;
            ...
            StreamType::Other(crate::handle::wisp::twisp::STREAM_TYPE) => {
                ...
                Ok(ClientStream::Pty(Box::new((cmd, pty))))
            }
...
impl WebSocketStreamWrapper {
    ...
    pub async fn write(
        ...
        self.0.send(Message::binary(payload)).await
    }
