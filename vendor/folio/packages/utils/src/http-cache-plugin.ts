// HTTP cache plugin for FolioFetchHandler.
//
...
export const CACHE_NAME = "folio-http-cache-v3";
const DEFAULT_MAX_ENTRY_BYTES = 2 * 1024 * 1024;
const STREAMING_DESTINATIONS = new Set<RequestDestination>(["audio", "video"]);
 ...
 * headers, not yet through folio's response-header rewriter.
 */
function responseIsStorable(
	...
	if (STREAMING_DESTINATIONS.has(destination)) return false;
	if (headers.has("set-cookie") || headers.has("content-range")) return false;
	...
	if (!NULL_BODY_STATUSES.has(status) && method !== "HEAD") {
		...
		if (!Number.isFinite(size) || size < 0 || size > maxEntryBytes) return false;
	}
...
function buildCacheKeyRequest(
	...
	const cacheKeyUrl =
		"https://fl-cache.invalid/" + encodeURIComponent(parsedUrl);
...
function strippedHeadersFromStored(stored: Response): Headers {
	...
	for (const [k, v] of stored.headers.entries()) {
		...
		if (lower === STORED_AT_HEADER || lower === "set-cookie") continue;
		try {
			out.append(k, v);
		} catch {}
	}
...
export interface HttpCachePluginOptions {
	...
	cacheName?: string;
	/** Maximum body size buffered into Cache API. Defaults to 2 MiB. */
	maxEntryBytes?: number;
}

/**
 * RFC-9111-ish HTTP cache for FolioFetchHandler.
 *
...
export class HttpCachePlugin extends ManagedPlugin {
	...
	private cameFromCache = new WeakMap<FolioFetchRequest, true>();
	private cacheKeys = new WeakMap<FolioFetchRequest, Request>();
	...
	install(frame: Frame): void {
		...
		this.tap(hooks.request, async (ctx, props) => {
			...
			this.cacheKeys.set(req, cacheKey);
			...
			const storedLength = Number.parseInt(
				stored.headers.get("content-length") ?? "",
				10
			);
			if (
				!isNullBody &&
				(!Number.isFinite(storedLength) || storedLength > this.maxEntryBytes)
			) {
		...
		this.tap(hooks.preresponse, async (ctx, props) => {
			...
			const cacheKey = this.cacheKeys.get(req);
			if (!cacheKey) return;
			...
			try {
				...
				console.warn("[folio-http-cache] cache.put failed:", err);
			}
		});
	}
	...
	async bust(): Promise<boolean> {
		try {
			...
			console.error("[folio-http-cache] bust failed:", err);
			return false;
		}
	}
}
