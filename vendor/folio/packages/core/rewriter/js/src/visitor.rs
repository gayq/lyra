...
macro_rules! audit_skip {
	($($t:tt)*) => {};
}
#[allow(unused)]
macro_rules! skip_field {
	($($t:tt)*) => {};
}
	...
	E: UrlRewriter,
{
	...
	fn handle_computed_member_expression(&mut self, it: &ComputedMemberExpression<'data>) {
		if self.flags.disable_computed_wrap {
			return;
		}
		match &it.expression {
	...
	fn recurse_object_assignment_target(
		...
		if let Some(r) = &s.rest {
			...
			match &r.target {
				...
				_ => panic!("unexpected object assignment rest target... /ᐠ - ˕ -マ"),
			}
		}
					...
					match &p.name {
						...
						PropertyKey::StringLiteral(s) => {
							if UNSAFE_GLOBALS.contains(&s.value.to_string().as_str()) {
								self.jschanges
									.add(rewrite!(s.span(), RewriteProperty { ident: s.value }));
							}
						}

						// this is really annoying, we have to list out all the things that *aren't* expressions, you can't just check if it is one
						// otherwise { 0:location } rewrites to { folio$prop(0):location } which is obviously invalid syntax
	...
	fn recurse_array_assignment_target(
		...
		for elem in (&s.elements).into_iter().flatten() {
			match elem {
			...
			BindingPatternKind::ObjectPattern(p) => {
				...
				if let Some(r) = &p.rest {
					match &r.argument.kind {
						...
						_ => panic!("unexpected object binding rest target... /ᐠ - ˕ -マ"),
					}
				}
			}
	...
	fn handle_for_of_in(
		...
		if let ForStatementLeft::VariableDeclaration(v) = &left {
			...
			// and the actual name of the variable to $folio$temploc so we can set it back later
	...
	E: UrlRewriter,
{
		...
		match &it {
			MemberExpression::StaticMemberExpression(s) => {
				...
				if s.property.name == "postMessage" && !matches!(&s.object, Expression::Super(_)) {
					self.jschanges
						.add(rewrite!(s.object.span(), WrapPostMessage));
	...
	fn visit_call_expression(&mut self, it: &CallExpression<'data>) {
		audit_skip!(
			it.callee,
			"top(0): none of the unsafe globals can be called as functions, other than eval which we handle above"
		);
	...
	fn visit_export_named_declaration(&mut self, it: &ExportNamedDeclaration<'data>) {
		...
		audit_skip!(
			it.declaration,
			"export {x} is safe because you can only export locals, which the only unsafe `top/parent` obviously can't be"
		);
		audit_skip!(
			it.specifiers,
			"export {x as y} is safe because you can only export locals"
		);
	}
	...
	fn visit_object_expression(&mut self, it: &ObjectExpression<'data>) {
		...
		audit_skip!(
			it.properties,
			"custom shorthand rewrite scans all properties before the normal object walker"
		);
	}
	...
	fn visit_function(
		...
		audit_skip!(
			it.params,
			"parameters are recursively visited with destructuring cleanup tracking"
		);
	...
	fn visit_for_of_statement(&mut self, it: &oxc::ast::ast::ForOfStatement<'data>) {
		audit_skip!(
			it.left,
			"loop left side is handled by handle_for_of_in with destructuring cleanup tracking"
		);
		...
		if matches!(it.operator, UnaryOperator::Typeof) {
			match it.argument {
				Expression::Identifier(_) => {
					...
					audit_skip!(
						it.argument,
						"safe, identifier tree cannot expand into an escape"
					);
					return;
				}
				_ => {
	...
	fn visit_update_expression(&mut self, it: &UpdateExpression<'data>) {
		...
		match it.argument {
			SimpleAssignmentTarget::AssignmentTargetIdentifier(_) => {
				...
				audit_skip!(
					it.argument,
					"ident++ would need $wrap(ident)++ which is invalid syntax; arithmetic on location coerces to NaN and assigns the string back, which navigates only to a non-attacker-controlled URL"
				);
				return;
			}
			_ => {}
	...
	fn visit_assignment_expression(&mut self, it: &AssignmentExpression<'data>) {
		audit_skip!(
			it.left,
			"left side is handled manually to preserve assignment/destructuring rewrites"
		);
		match &it.left {
			AssignmentTarget::AssignmentTargetIdentifier(s) => {
				// location = ...
