...
pub enum JsChangeType<'alloc: 'data, 'data> {
	...
	MetaFn,
	/// replace span with `((t)=>$folio$tryset(${name},"${op}",t)||(${name}${op}t))(`
	AssignmentLeft {
		name: Atom<'data>,
	...
	fn into_low_level(
		...
		match self.ty {
			...
			Ty::WrapObjectAssignmentLeft {
				...
				for id in restids {
					let _ = write!(steps, "{}({}),", cfg.cleanrestfn, id.as_str());
				}
			...
			Ty::MetaFn => LL::replace(transforms![
				&cfg.metafn,
				"(import.meta,\"",
				&flags.base,
				"\")"
	...
	fn cmp(&self, other: &Self) -> std::cmp::Ordering {
		...
		match self.span.start.cmp(&other.span.start) {
			Ordering::Equal => match (&self.ty, &other.ty) {
				(Ty::CleanFunction { .. } | Ty::ScramErrFn { .. }, _) => Ordering::Less,
