...
if [ -f out/.build-hash ] && [ -f ../../dist/folio.wasm ] && [ "$SRC_HASH" != "unknown" ] && grep -q "$SRC_HASH" out/.build-hash; then
...
(
	...
	cargo +nightly-2026-06-30 build --release --target wasm32-unknown-unknown \
...
cp rewriter/wasm/out/optimized.wasm dist/folio.wasm
echo "$SRC_HASH" > rewriter/wasm/out/.build-hash || true
echo "rewriter build complete!! (˵◝ ⩊  ◜˵マ"
