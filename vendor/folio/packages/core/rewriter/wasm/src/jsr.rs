...
impl UrlRewriter for WasmUrlRewriter {
	fn rewrite(
		...
		let rewritten = self
			.0
			.call2(&JsValue::NULL, &url.into(), &JsValue::from_bool(module))
			.map_err(RewriterError::from)?
			.as_string()
...
pub fn create_js() -> JsRewriter {
	Rewriter::new()
}

pub fn get_url_rewriter(func: Object) -> Result<WasmUrlRewriter> {
	Ok(WasmUrlRewriter(func.dyn_into().map_err(|_| {
		RewriterError::not_fn("folio.codec.encode")
	})?))
}
