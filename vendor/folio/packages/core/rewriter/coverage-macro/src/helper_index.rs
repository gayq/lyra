...
fn build_index_from(src: &str) -> HelperIndex {
	...
	loop {
		...
		for name in names {
			...
			for (arg_idx, a) in new_args.iter_mut().enumerate() {
				let (fully, set, all) = analyze_helper_arg(&w, arg_idx, &infos_snapshot, &graph);
...
fn extract_method(f: &syn::ImplItemFn) -> Option<WorkingHelper> {
	...
	for (pos, arg) in iter.enumerate() {
		...
		let Some(ast_type) = extract_ref_type_name(ty) else {
			continue;
		};
...
impl OriginMap {
	...
	pub fn set(&mut self, name: &str, origin: Origin) {
		self.map
			.insert(name.to_string(), Binding::with_origin(origin));
	}
	...
	pub fn get(&self, name: &str) -> Origin {
		self.map
			.get(name)
			.map_or(Origin::Unknown, |b| b.origin.clone())
	}
	...
	match inner {
		...
		Expr::Field(f) => {
			...
			if let syn::Member::Named(n) = &f.member {
				...
				let field = def.fields.iter().find(|fd| n == fd.name)?;
...
fn method_result_type(recv: Option<&str>, method: &str) -> Option<String> {
	...
	let recv = recv?;
	match (recv, method) {
		("ForStatementLeft" | "AssignmentTargetMaybeDefault", "as_assignment_target") => {
