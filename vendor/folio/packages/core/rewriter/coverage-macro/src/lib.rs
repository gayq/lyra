...
use crate::analyze::{PathKind, Witness, analyze_with_helpers_typed, extract_receiver_and_field};
...
pub fn coverage_checked(attr: TokenStream, item: TokenStream) -> TokenStream {
	...
	let Some(def) = graph.nodes.get(node_name.as_str()) else {
		return error_at_span(
			func.sig.ident.span(),
			&format!(
				"coverage_checked: unknown AST node type `{node_name}` (add it to ast_table.rs)"
			),
		)
		.into();
	};
	...
	for f in def.fields {
		...
		for (idx, p) in paths.iter().enumerate() {
			let covered = p.covered.all || p.covered.fields.iter().any(|c| c == f.name);
			if !covered {
...
impl<'ast> syn::visit::Visit<'ast> for SkipChecker<'_> {
	fn visit_macro(&mut self, m: &'ast syn::Macro) {
		...
		if name == "skip_field" {
			if let Some((_, fname)) = extract_receiver_and_field(&m.tokens)
				&& let Some(field) = self.fields.iter().find(|f| f.name == fname)
				&& self.graph.field_in_r(field)
			{
				let msg = format!(
			...
			if std::env::var("COVERAGE_AUDIT").is_ok()
				&& let Some((_, fname)) = extract_receiver_and_field(&m.tokens)
			{
				eprintln!(
...
fn expand_block_inner(block: &syn::Block, def: NodeDef, out: &mut TokenStream2) {
	let brace = proc_macro2::Group::new(proc_macro2::Delimiter::Brace, {
...
fn expand_token_stream(input: TokenStream2, def: NodeDef) -> TokenStream2 {
	...
	while i < tts.len() {
		if i + 2 < tts.len()
			&& let (TokenTree::Ident(name), TokenTree::Punct(bang), TokenTree::Group(g)) =
...
fn try_expand_marker(name: &str, inner: TokenStream2, def: NodeDef) -> Option<TokenStream2> {
	match name {
		"walk_all" => {
...
fn expand_walk_field_ctx(inner: TokenStream2, def: NodeDef) -> Option<TokenStream2> {
