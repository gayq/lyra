...
export default function (client: FolioClient, self: Self) {
	...
	function xhrIncludesCredentials(this: XMLHttpRequest, url: unknown): boolean {
		if (this.withCredentials) return true;
		try {
			return new URL(String(url), client.url.href).origin === client.url.origin;
		} catch {
			return false;
		}
	}
	...
	client.Proxy("XMLHttpRequest.prototype.open", {
		apply(ctx) {
			if (ctx.args[1]) {
				const credentials = xhrIncludesCredentials.call(ctx.this, ctx.args[1])
					? "include"
					: undefined;
	...
	client.Proxy("XMLHttpRequest.prototype.send", {
		apply(ctx) {
			...
			if (!client.flagEnabled("syncxhr")) {
				console.warn(
					"ignoring request because synchronous xhr is disabled /ᐠ - ˕ -マ"
				);
			...
			while (view.getUint8(0) === 0) {
				if (performance.now() - now > 1000) {
					throw new Error("xhr timed out /ᐠ - ˕ -マ");
				}
				/* spin */
			}
...
export function unrewriteLinkHeader(header: string, context: FolioContext) {
