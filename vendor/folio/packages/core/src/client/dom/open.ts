...
export default function (client: FolioClient) {
	client.Proxy("window.open", {
		apply(ctx) {
			...
			if (!(FOLIOCLIENT in realwin)) {
				// i don't believe it's possible for a just-opened window to already have folio loaded but just in case
	...
	client.Trap("window.frameElement", {
		get(ctx) {
			...
			if (win[FOLIOCLIENT]) {
				// then this is a subframe in a folio context, and it's safe to pass back the real iframe
				return f;
			} else {
