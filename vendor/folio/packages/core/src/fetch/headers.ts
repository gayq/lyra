...
export async function rewriteResponseHeaders(
...
): Promise<FolioHeaders> {
	const headers = FolioHeaders.fromRawHeaders(rawHeaders);
	...
	// folio runtime can use features that permissions-policy blocks
...
export function rewriteRequestHeaders(
	...
	// avoid leaking the folio referer
...
function applyFetchMetadataHeaders(
	...
	// `fl$mode` value the client-side proxy stamped onto the URL when present
	...
	// rely on signals propagated from the page (the `fl$cred` URL param set by
 ...
 * from:
 *
 * - the `fl$cred` URL parameter, set by the client-side fetch proxy when the
 ...
 * request URL's relationship to the page, and folio has rewritten the URL
...
function computeFetchMode(
	...
	// folio's proxy origin and is meaningless to the destination.
...
function resolveFetchInitiatorUrl(
	...
	// The candidate URL is outside folio's logical space (e.g. the runway
