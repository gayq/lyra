...
import { domainMatches, isPublicSuffix } from "./domain";
...
export class CookieJar {
	...
	setCookies(cookieString: string, url: URL) {
		...
		const responseHostname = url.hostname.toLowerCase().replace(/\.$/, "");
		...
		for (const parsedCookie of parsedCookies) {
			...
			if (typeof parsedCookie.domain === "string" && parsedCookie.domain) {
				const domain = parsedCookie.domain
					.toLowerCase()
					.replace(/^\.+|\.+$/g, "");
				if (!domainMatches(responseHostname, domain) || isPublicSuffix(domain)) {
					continue;
				}
				parsedCookie.domain = domain;
			} else if (parsedCookie.domain) {
				continue;
			}

			const sameSite = parsedCookie.sameSite?.toLowerCase();
			...
			if (!cookie.domain) cookie.domain = responseHostname;
		...
		while (key !== undefined) {
			...
			if (bucket) {
				for (const cookie of bucket) {
					...
					if (httpOnlyOnly && !cookie.httpOnly) continue;
	...
	dumpForJs(): string {
		const visible: Record<string, Cookie> = {};
		for (const [id, cookie] of Object.entries(this.cookies)) {
			if (!cookie.httpOnly) visible[id] = cookie;
		}
		return JSON_stringify(visible);
	}
}
